...
const cloudMode = config?.useSupabase !== false;
...
async function save(next, expectedRevision = revision) {
  ...
  if (cloudMode) {
    ...
    try {
      ...
      return true;
    } catch (error) { if (version === authVersion) toast(globalThis.SantriPulangSupabase.message(error)); return false; }
...
function initAuth() {
  if (cloudMode) {
    $('#auth-title').textContent = 'Masuk admin';
...
$('#auth-form').addEventListener('submit', async event => {
  ...
  if (cloudMode) {
    ...
    catch (error) { $('#auth-message').textContent = globalThis.SantriPulangSupabase.message(error); }
...
async function startCloud() {
  try {
    if (!globalThis.SantriPulangSupabase) throw new Error('Helper Supabase tidak termuat. Pastikan supabase-service.js tersedia, lalu muat ulang.');
    cloud = await globalThis.SantriPulangSupabase.create(config);
    initAuth();
    const stopCloud = cloud.watch(snapshot => {
      if (authenticated && snapshot.revision <= revision) return;
    ...
    window.addEventListener('pagehide', () => { stopCloud(); lockCloud('Muat ulang untuk masuk kembali.'); void cloud.logout().catch(() => {}); }, { once: true });
    $('.local-badge').textContent = 'Mode Supabase';
    $('.profile strong').textContent = 'Admin Supabase';
    $('.table-footer span:last-child').textContent = 'Data bersama di Postgres';
    $('#history-view .card-heading p').textContent = 'Pencatatan bersama seluruh pengurus yang diizinkan.';
    $('#settings-form .settings-note').textContent = 'Prototipe Postgres satu JSON; perlu koneksi server. Batas 200.000 karakter / 700 KB termasuk format JSON database. Ekspor berisi data pribadi: simpan secara aman.';
  } catch (error) {
    authBlocked = true; $('#auth-submit').disabled = true;
    $('#auth-title').textContent = 'Konfigurasi Supabase belum siap';
    $('#auth-message').textContent = (globalThis.SantriPulangSupabase?.message(error) || error.message) + ' Lihat README.md, lalu muat ulang. Data lokal tidak dibaca atau dikirim.';
  }
}
