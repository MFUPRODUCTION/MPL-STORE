...
function appHarness(crypto, saved = {}, integration = { SantriPulangConfig: { useFirebase: false } }) {
...
(async () => {
  ...
  const c = appHarness(undefined, { 'santripulang-v1': '{private-local-data}' }, {
    SantriPulangConfig: { useFirebase: true }, onRead() { reads++; }, onWrite() { writes++; },
    SantriPulangFirebase: { message: e => e.message, create: async () => ({
